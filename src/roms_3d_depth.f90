...
!                  +theta_b*(cff2*tanh(theta_s*(sc_r(k)+0.5))-0.5)
      ...
      !print*, "roms depth at (1,1,:)",z_w(1,1,:),h(1,1),zeta(1,1)

      do k=1,Nz
         drf_r(:,:,k-1)=1d0/Hz(:,:,k) !reverse roms vertical grid to replace k=0 at the surface
      enddo
         drf_r(:,:,-1)=drf_r(:,:,0)
         drf_r(:,:,Nz)=drf_r(:,:,Nz-1)

      !print*, "roms_3d_depth.f90, 1/drf_r (1,1,:)",1/drf_r(1,1,:)
