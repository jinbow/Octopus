...
!$OMP PARALLEL DO PRIVATE(ip) SCHEDULE(dynamic)
    do ip =1,Npts
        ...
        if (hFacC(pi2f(ip,IPP),pj2f(ip,IPP),pk2f(ip,IPP))<1e-5) then
            xyz(ip,1,IPP)=reflect_x(pi2f(ip,IPP),pj2f(ip,IPP),pk2f(ip,IPP)) !semi-reflective boundary
            xyz(ip,2,IPP)=reflect_y(pi2f(ip,IPP),pj2f(ip,IPP),pk2f(ip,IPP)) !semi-reflective boundary
        endif
